...
const CoursePage = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const response = await fetch(`https://wb132r23-8000.inc1.devtunnels.ms/get-course/${id}`);
        if (!response.ok) throw new Error("Failed to fetch course data");
        const data = await response.json();
        
        if (data && data.title) {
          setCourse(data);
          // Don't change the URL - keep the original ID for API compatibility
        } else {
          setCourse(null);
        }
      } catch (error) {
  ...
  // Parse modules from tableOfContents with realistic durations
  const modules = [];
  if (course.tableOfContents && Array.isArray(course.tableOfContents)) {
    const lessonDurations = ["15 min", "25 min", "18 min", "22 min", "20 min", "30 min", "12 min", "35 min"];
    
    // Group into modules of 3 for display purposes
    for (let i = 0; i < course.tableOfContents.length; i += 3) {
      const moduleIndex = Math.floor(i / 3);
      const moduleTitle = `Module ${moduleIndex + 1}: ${course.tableOfContents[i].split(':')[0] || 'Course Content'}`;
      
      const lessons = [];
      let totalMinutes = 0;
      
      for (let j = i; j < Math.min(i + 3, course.tableOfContents.length); j++) {
        const content = course.tableOfContents[j];
        const parts = content.split(':');
        const title = parts.length > 1 ? parts[1].trim() : content.trim();
        const duration = lessonDurations[j % lessonDurations.length];
  ...
  // Use the URL parameter (which should be the course ID) for enrollment
          ...
          <div className="lg:w-2/3">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 border border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {course.tableOfContents && course.tableOfContents.slice(0, 6).map((content, index) => (
              ...
              <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-4 mb-6">
                <div className="flex flex-wrap gap-6 text-sm text-gray-600">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="font-semibold">{course.tableOfContents ? course.tableOfContents.length : 0} lessons</span>
                  </div>
                  <div className="flex items-center">
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-sm text-gray-500">{module.lessons.length} lessons • {module.totalDuration}</p>
                        </div>
                      </div>
                      <svg 
                      ...
                      <div className="bg-gray-50 border-t border-gray-200">
                        {module.lessons.map((lesson, idx) => (
                          <div key={idx} className="flex justify-between items-center py-4 px-6 hover:bg-white transition-colors border-b border-gray-100 last:border-b-0">
                            ...
                            <span className="text-sm text-gray-500 bg-gray-200 px-2 py-1 rounded-full">{lesson.duration}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
              ...
              <div className="flex flex-wrap gap-3">
                {(course.skills || course.category?.split(',') || ["Web Development", "Programming", "Problem Solving"]).map((skill, index) => (
